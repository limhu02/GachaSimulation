...
public class GachaController {
	...
	public ModelAndView gachaItem(HttpSession session) {
		ItemBox box = this.boxService.getBoxByCode("1"); //선택한 박스 가져오기 (임시)
		
		
		List<Item> itemList=this.itemService.getItemByBox("1"); //해당 박스코드에 소속된 아이템 목록 가져오기
		ModelAndView mav = new ModelAndView("gacha"); //뷰 생성
		...
		if(item.getRate()<=2) result.setHighitemcount(result.getHighitemcount()+1); //실행 할 때 마다 결과 연산
