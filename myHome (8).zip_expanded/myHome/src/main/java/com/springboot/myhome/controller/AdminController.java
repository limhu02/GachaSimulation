...
import com.springboot.myhome.service.AdminService;
...
public class AdminController {
	...
	@GetMapping(value="/admin/orderModifyFromList.html")
	public ModelAndView modifyfromlist(String user_id, Integer sale_id,
		...
		this.adminService.modifyOrderState(om);
		ModelAndView mav = new ModelAndView("index");
		mav.addObject("BODY","orderModifyFromListResult.jsp");
		return mav;
	}
	
	@GetMapping(value="/admin/orderStateSearch.html")
	public ModelAndView searchByState(Integer state, Integer PAGE) {
		...
		int totalCount = this.adminService.getOrdersByStateCount(state);
...
//		List<OrderModify> orderList = this.adminDao.getOrdersByState(st);
		List<OrderModify> orderList = this.adminService.getOrdersByState(PAGE, state);
	...
	public ModelAndView orderlist(Integer PAGE) {
		...
		List<OrderModify> allOrders = this.adminService.getAllOrders(PAGE);
	...
	@GetMapping(value="/admin/orderModify.html")
	...
	@GetMapping(value="/admin/deliverymenu.html")
	...
	@PostMapping(value="/admin/memberUpdate.html")
	public ModelAndView update(String ID,String PWD,String ADDR,String PHONE,
									Integer GRADE) {
		ModelAndView mav = new ModelAndView("index");
	...
	public ModelAndView findmember(String ID,
			...
			int totalCount = this.adminService.findMemberCount(user);
			int pageCount = totalCount / 5;
			if(totalCount % 5 != 0) pageCount++;
			user.setStart(start); user.setEnd(end);
			//List<User> memberList = this.adminDao.findMember(user);
			List<User> memberList = this.adminService.findMember(user);
	...
	@GetMapping(value="/admin/findpage.html")
